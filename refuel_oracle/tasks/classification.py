...
class ClassificationTask(BaseTask):
    ...
    DEFAULT_OUTPUT_FORMAT_PROMPT = "You will return the answer in JSON format with two keys: {\"answered\": \"can you answere this question. say YES or NO\", \"label\": \"the correct label\"}"
    PROMPT_TEMPLATE = "{prefix_prompt}\n{task_prompt}\n\n{output_prompt}\n\nSome examples with their output answers are provided below:\n{seed_examples}\n{current_example}"
    PROMPT_TEMPLATE_VARIABLES = ["prefix_prompt",
                                 "task_prompt",
                                 "output_prompt",
                                 "seed_examples",
                                 "current_example"]
    ...
    def parse_llm_response(self, prompt: str, response: Generation) -> LLMAnnotation:
        ...
        if "output_format_prompt" in self.config.keys():
            ...
            try:
                ...
                logger.error(f"Error parsing LLM response: {response.text}. Encountered exception: {e}")
