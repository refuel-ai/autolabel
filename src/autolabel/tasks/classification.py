...
class ClassificationTask(BaseTask):
    ...
    GENERATE_EXPLANATION_PROMPT = "You are an expert at providing a well reasoned explanation for how to determine the output for a given task. \n\nBEGIN TASK DESCRIPTION\n{task_guidelines}\nEND TASK DESCRIPTION\nYou will be given an input example and the corresponding output. Your job is to provide step-by-step reasoning on how to arrive at the given output, from scratch.\nDo not simply justify why the output is correct or verify that it is correct, but rather explain how to arrive at the output. Your explanation should not be more than 4 sentences. {label_format}\n{labeled_example}\nExplanation: "
