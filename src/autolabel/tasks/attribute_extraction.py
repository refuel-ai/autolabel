...
class AttributeExtractionTask(BaseTask):
    ...
    GENERATE_EXPLANATION_PROMPT = "You are an expert at providing a well reasoned explanation for how to determine the output for a given task. \n\nBEGIN TASK DESCRIPTION\n{task_guidelines}\nEND TASK DESCRIPTION\nYou will be given an input example and the output for one of the attributes. Provide a step-by-step explanation for how to arrive at the given output for the current attribute for the task above. Do not begin your explanation with the correct label. Instead, start with the step by step explanation and only end with the correct label.\nYour explanation should be at most two sentences.{label_format}\n{labeled_example}\nCurrent Attribute:{attribute}.\nExplanation: "
